function showDebugBanner(label, detail) {
  const div = document.createElement('div');
  div.style.cssText = 'position:fixed;bottom:0;left:0;right:0;background:#c0392b;color:white;padding:10px;font-size:11px;z-index:99999;max-height:250px;overflow:auto;white-space:pre-wrap;';
  div.innerText = `[${label}] ${detail}`;
  document.body.appendChild(div);
}
window.onerror = function(msg, url, line, col, error) {
  showDebugBanner('onerror', `${msg} | línea ${line} | ${url}`);
  return false;
};
window.addEventListener('unhandledrejection', function(event) {
  showDebugBanner('promise-rechazada', event.reason?.message || JSON.stringify(event.reason));
});
...
/* A veces el script del CDN de Supabase no termina de cargar/ejecutar a tiempo
   (conexión lenta, bloqueador de contenido, hiccup del CDN), y window.supabase
   no existe todavía. En vez de romper toda la app en silencio, si esto pasa
   recargamos la página automáticamente UNA sola vez (usamos sessionStorage
   para no entrar en un ciclo infinito de recargas si el problema persiste). */
if (!window.supabase) {
  const yaReintento = sessionStorage.getItem("credit_cdn_retry");
  if (!yaReintento) {
    sessionStorage.setItem("credit_cdn_retry", "1");
    location.reload();
    throw new Error("Recargando la página para reintentar cargar Supabase…");
  } else {
    sessionStorage.removeItem("credit_cdn_retry");
    showDebugBanner(
      "supabase-cdn",
      "No se pudo cargar la librería de Supabase (CDN) después de reintentar. Revisa tu conexión a internet y vuelve a abrir la app."
    );
  }
} else {
  sessionStorage.removeItem("credit_cdn_retry");
...
/* Un usuario nuevo arranca completamente vacío: sin tarjetas, sin transacciones
   y sin dinero total precargado. Nada de datos de ejemplo/hardcodeados. */
const DEFAULT_STATE = {
  dineroTotalHistory: [],
  cards: [],
...
let helpOpen = false; // controla la sección "¿Qué significa esto?" dentro de Ajustes
...
/* Sincroniza el estado completo con Supabase.
   ...
   IMPORTANTE: cada operación revisa explícitamente su campo "error" y lo lanza
   si existe. El cliente de Supabase NO lanza excepciones por errores de base de
   datos (tabla inexistente, política RLS que rechaza la escritura, etc.) — solo
   devuelve { data, error } sin interrumpir el await. Si no se revisa "error" a
   mano, una escritura puede fallar en silencio y el resto del código (trySync)
   la trataría como éxito, marcando la copia local como "ya sincronizada" cuando
   en realidad nada llegó al servidor. */
async function persistToSupabase(next) {
  if (!currentUser) return false;
  const userId = currentUser.id;
  try {
    const r1 = await sb.from("configuraciones").upsert(
      { user_id: userId, tarjetas: next.cards, categorias_gasto: next.expenseCategories, updated_at: new Date().toISOString() },
      { onConflict: "user_id" }
    );
    if (r1.error) throw r1.error;

    const r2 = await sb.from("ingresos").delete().eq("user_id", userId);
    if (r2.error) throw r2.error;
    if (next.dineroTotalHistory.length) {
      const r3 = await sb.from("ingresos").insert(
    ...
    const r4 = await sb.from("egresos").delete().eq("user_id", userId);
    if (r4.error) throw r4.error;
    if (next.transactions.length) {
      const r5 = await sb.from("egresos").insert(
    ...
    console.error("Error guardando en Supabase (queda pendiente en la cola local para reintentar)", e);
    showDebugBanner('persistToSupabase', e?.message || e?.details || JSON.stringify(e));
    return false;
  }
}

/* ---------------------------- PERSISTENCIA LOCAL (IndexedDB-first) ----------------------------
   Arquitectura anti-pérdida-de-datos para iOS: cada cambio se escribe en
   IndexedDB de forma INMEDIATA (writeLocalState), antes de siquiera intentar
   tocar la red. Se usa IndexedDB en vez de localStorage porque en iOS, cuando
   la PWA instalada se cierra abruptamente desde el selector de ventanas,
   localStorage no garantiza que el volcado a disco termine antes de que el
   proceso muera — IndexedDB sí ofrece esa garantía de escritura transaccional.
   Supabase es un espejo/backup que se sincroniza justo después, y también al
   recuperar el foco (visibilitychange), al recuperar conexión (online) y al
   abrir la app — nunca dependiendo de beforeunload/cierre, que iOS no
   garantiza ejecutar.
   Si la sincronización falla, el registro local queda marcado "dirty": true
   (cola pendiente) y se reintenta automáticamente más adelante, sin perder
   nada mientras tanto porque la fuente de verdad inmediata ya es IndexedDB. */
const IDB_DB_NAME = "credit_v1_db";
const IDB_STORE = "state";
let idbPromise = null;

/* Abre (o crea) la base de IndexedDB una sola vez y reutiliza la conexión. */
  ...
  idbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(IDB_STORE)) {
        db.createObjectStore(IDB_STORE); // key = userId, value = { state, dirty, updatedAt }
      }
    };
...
/* Lee el respaldo local del usuario. Devuelve una Promise<{state, dirty, updatedAt} | null>. */
async function readLocalState(userId) {
  try {
    ...
    return await new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        resolve(parsed && parsed.state ? parsed : null);
      };
      req.onerror = () => reject(req.error);
    });
  } catch (e) {
    console.error("Error leyendo respaldo local (IndexedDB)", e);
    showDebugBanner('readLocalState', e?.message || JSON.stringify(e));
    return null;
  }
}

/* Escribe el respaldo local del usuario dentro de una transacción de IndexedDB.
   Devuelve una Promise que se resuelve cuando la transacción confirma. */
async function writeLocalState(userId, next, dirty) {
  try {
    const db = await openLocalDB();
    await new Promise((resolve, reject) => {
      const tx = db.transaction(IDB_STORE, "readwrite");
      tx.objectStore(IDB_STORE).put({ state: next, dirty: !!dirty, updatedAt: Date.now() }, userId);
      tx.oncomplete = () => resolve();
      tx.onerror = () => reject(tx.error);
      tx.onabort = () => reject(tx.error);
    });
  } catch (e) {
    console.error("Error guardando respaldo local (IndexedDB)", e);
    showDebugBanner('writeLocalState', e?.message || JSON.stringify(e));
  }
}
...
/* Intenta subir a Supabase lo que haya pendiente en la cola local del usuario
   actual. Es seguro llamarla muchas veces seguidas: si ya hay una sincronización
   en curso, o no hay nada pendiente, no hace nada. */
async function trySync() {
  if (!currentUser || syncInFlight) return;
  const local = await readLocalState(currentUser.id);
  if (!local || !local.dirty) return;
  if (typeof navigator !== "undefined" && navigator.onLine === false) return;
  syncInFlight = true;
  const ok = await persistToSupabase(local.state);
  if (ok) {
    // Solo limpia la marca "pendiente" si nadie volvió a guardar un cambio más
    // nuevo mientras la sincronización estaba en curso (si sí, ese nuevo
    // guardado ya dejó su propia marca "dirty" para reintentar después).
    const stillCurrent = await readLocalState(currentUser.id);
    if (stillCurrent && stillCurrent.updatedAt === local.updatedAt) {
      await writeLocalState(currentUser.id, local.state, false);
    }
  }
  syncInFlight = false;
}

/* Agrupa sincronizaciones muy seguidas (varios taps rápidos) en una sola
   llamada a Supabase, sin retrasar nunca el guardado local. */
...
function saveState(next) {
  state = next;
  render(); // UI optimista: se refleja de inmediato, sin esperar a IndexedDB ni a Supabase
  if (currentUser) writeLocalState(currentUser.id, next, true); // 1) guardado inmediato en IndexedDB (no se espera; la promesa corre en paralelo a la UI)
  scheduleSync(); // 2) sincroniza con Supabase (espejo/backup) en segundo plano; si falla, queda pendiente y se reintenta solo
}
    ...
    <div class="panel">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        ...
        <div style="grid-column:1/-1;">
          ...
          <select id="tx-card" ${state.cards.length === 0 ? "disabled" : ""}>${
            state.cards.length === 0
              ? `<option value="">Agrega una tarjeta primero (pestaña Tarjetas)</option>`
...
async function bootAfterLogin(user) {
  ...
  if (local) {
    // Arranque instantáneo desde el respaldo local (IndexedDB): es la fuente de
    // verdad inmediata, así que la app funciona aunque no haya red o Supabase
    // tarde en responder.
    state = local.state;
    dataLoading = false;
    render();
    if (local.dirty) {
      // Había cambios pendientes de una sesión anterior (p. ej. la app se cerró
      // en iOS antes de sincronizar): se reintenta subirlos ahora.
      trySync();
    } else {
      // Sin pendientes locales: refresca en segundo plano por si hay datos más
      // recientes en Supabase (otro dispositivo), sin bloquear ni pisar la UI.
      refreshFromSupabaseInBackground(user.id);
    }
    return;
  }

  // Primer inicio de este usuario en este dispositivo/navegador: no hay copia
  // local todavía, así que se hidrata desde Supabase y esa copia queda guardada
  // localmente para que la próxima vez el arranque sea instantáneo.
  dataLoading = true;
  renderAuth();
  state = await loadStateFromSupabase();
  await writeLocalState(user.id, state, false);
  dataLoading = false;
  render();
}

async function refreshFromSupabaseInBackground(userId) {
  try {
    const fresh = await loadStateFromSupabase();
    const local = await readLocalState(userId);
    // No pisa el estado si en el ínterin se generaron cambios locales pendientes
    // o si el usuario ya cambió de sesión.
    if (currentUser && currentUser.id === userId && (!local || !local.dirty)) {
      state = fresh;
      await writeLocalState(userId, fresh, false);
      render();
    }
  } catch (e) {
    console.error("No se pudo refrescar desde Supabase; se conserva la copia local", e);
  }
}
  ...
  app.innerHTML = `
    ...
    <div class="modal-overlay" id="settings-overlay">
      <div class="modal-sheet">
        ...
        <button id="settings-help-toggle" class="row" style="cursor:pointer;background:transparent;border:none;border-bottom:none;width:100%;text-align:left;padding-top:16px;margin-top:16px;border-top:1px solid var(--borderSoft);">
          <span style="display:flex;align-items:center;gap:8px;font-size:13px;color:var(--text);">
            <span class="icon-btn" style="width:26px;height:26px;border-radius:8px;color:var(--amber);">${SVG_INFO}</span>
            ¿Qué significa esto?
          </span>
          <span class="dim" style="font-size:15px;">${helpOpen ? "▾" : "›"}</span>
        </button>
        ${helpOpen ? `
        <div id="settings-help-content" style="padding-top:4px;">
          ${HELP_CONTENT.map((h) => `
            <div style="padding:10px 0;border-bottom:1px solid var(--borderSoft);">
              <div style="font-size:13px;font-weight:600;margin-bottom:4px;">${esc(h.title)}</div>
              <div class="dim" style="font-size:12.5px;line-height:1.55;">${esc(h.text)}</div>
            </div>`).join("")}
        </div>` : ""}
      </div>
    </div>` : ""}
...
const SVG_INFO = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9.5"/><path d="M12 11v6"/><path d="M12 7.5h.01"/></svg>`;

/* Contenido de la sección "¿Qué significa esto?" del modal de Ajustes. */
const HELP_CONTENT = [
  {
    title: "Dinero total",
    text: "No es tu sueldo ni lo que ganas al mes. Es lo que tienes disponible ahora mismo, en este momento, sumando y restando cada movimiento que registras — como una calculadora, no como un total que se reinicia cada quincena.",
  },
  {
    title: "¿Qué tarjeta uso?",
    text: "Antes de que hagas un gasto, te decimos qué tarjeta te conviene usar hoy. Consideramos cuándo cierra cada tarjeta, cuándo tienes que pagarla, qué tan cerca estás del límite y si ya tienes compromisos a meses sin intereses (MSI) pendientes — para que nunca gastes crédito pensando que es dinero libre.",
  },
  {
    title: "MSI (Meses Sin Intereses)",
    text: "Aunque hoy no pagues el total, un MSI ya es un compromiso a futuro. Lo contamos como si ya lo debieras, para que tus próximos meses no te agarren en curva.",
  },
];
...
function attachHandlers() {
  ...
  if (settingsOverlay) {
    settingsOverlay.addEventListener("click", (e) => {
      if (e.target === settingsOverlay) { settingsOpen = false; helpOpen = false; render(); }
    });
  }
  const settingsClose = document.getElementById("settings-close");
  if (settingsClose) settingsClose.addEventListener("click", () => { settingsOpen = false; helpOpen = false; render(); });

  const settingsLogoutBtn = document.getElementById("settings-logout-btn");
  if (settingsLogoutBtn) settingsLogoutBtn.addEventListener("click", () => { settingsOpen = false; helpOpen = false; handleSignOut(); });

  const settingsHelpToggle = document.getElementById("settings-help-toggle");
  if (settingsHelpToggle) settingsHelpToggle.addEventListener("click", () => { helpOpen = !helpOpen; render(); });
...
/* Reintentos automáticos de sincronización — nunca dependemos de beforeunload
   ni de eventos de cierre, que iOS no garantiza ejecutar antes de matar el
   proceso. En su lugar, se reintenta cada vez que la app recupera el foco,
   recupera conexión, o se vuelve a mostrar (incluye volver desde el selector
   de ventanas en iPhone). */
