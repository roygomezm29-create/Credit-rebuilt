...
/* Local-first: Supabase es un RESPALDO en segundo plano. Ninguna operación
   remota bloquea la UI ni muestra errores visuales — LocalStorage ya
   contiene la verdad de lo que el usuario ve, así que un fallo de red aquí
   solo se registra en consola y se reintenta solo (cola + evento "online"). */
async function runOp(op) {
  ...
  try {
    ...
    console.warn("Supabase (respaldo en segundo plano) no disponible; se reintentará", e);
    enqueueOp(op);
  }
}
...
/* ============================================================================
   LOCAL-FIRST: LocalStorage es la fuente primaria en el dispositivo.
   Cada acción del usuario se guarda aquí de inmediato (síncrono), por lo que
   sobrevive a que iOS mate la app desde la multitarea. Supabase queda como
   respaldo/sincronización en segundo plano (ver runOp más arriba).
============================================================================ */
const LOCAL_STATE_PREFIX = "credit_state_";
const LAST_USER_KEY = "credit_last_user";
const LAST_EMAIL_KEY = "credit_last_user_email";

function localStateKey(userId) { return LOCAL_STATE_PREFIX + userId; }

function saveLocalState(userId, s) {
  try { localStorage.setItem(localStateKey(userId), JSON.stringify(s)); } catch (e) { console.warn("No se pudo guardar el estado en LocalStorage", e); }
}
function loadLocalState(userId) {
  try {
    ...
    // Completa con la forma por defecto por si el caché es de una versión anterior.
...
/* Aplica un nuevo estado local de inmediato (UI optimista):
   1) Se guarda en LocalStorage de forma SÍNCRONA (fuente primaria).
   2) Se renderiza la UI al instante.
   La persistencia remota (Supabase) se dispara por separado, de forma
   puntual y sin bloquear, en cada manejador de evento. */
function applyState(next) {
  state = next;
  if (currentUser) saveLocalState(currentUser.id, next);
  render();
}
...
/* Entra a la app de forma LOCAL-FIRST: si ya existe un caché en este
   dispositivo para este usuario, se pinta al instante y Supabase solo
   sincroniza en segundo plano. Solo se espera la red la primerísima vez
   que este usuario inicia sesión en este dispositivo (sin caché todavía). */
...
/* Sincronización en segundo plano: primero sube lo pendiente de la cola
   offline y luego trae el estado fresco de Supabase, sin bloquear ni
   interrumpir lo que el usuario ya está viendo (pintado desde LocalStorage). */
async function syncFromSupabaseInBackground() {
  ...
  try {
    ...
    console.warn("Sincronización en segundo plano con Supabase no disponible", e);
  }
}
...
/* ---------------------------- INICIO ---------------------------- */
async function boot() {
  // 1) LECTURA INSTANTÁNEA: si hay un usuario y un estado cacheados en este
  //    dispositivo, se pintan de inmediato, ANTES de esperar ninguna
  //    respuesta de red. Esto es lo que evita que la app se vea reseteada o
  //    en blanco al reabrirla desde la multitarea de iOS.
  const cachedUserId = localStorage.getItem(LAST_USER_KEY);
  const cachedEmail = localStorage.getItem(LAST_EMAIL_KEY);
  let paintedFromCache = false;

  if (cachedUserId) {
    const cachedState = loadLocalState(cachedUserId);
    if (cachedState) {
      currentUser = { id: cachedUserId, email: cachedEmail || "" };
      state = cachedState;
      dataLoading = false;
  ...
  // 2) En paralelo/después, se valida la sesión real y se sincroniza con
  //    Supabase en segundo plano, sin bloquear ni reemplazar la UI ya visible.
  const { data: { session } } = await sb.auth.getSession();

  if (session) {
    const sameUserAsCache = paintedFromCache && session.user.id === cachedUserId;
    ...
    if (sameUserAsCache) {
      ...
      // Sesión válida pero sin caché local todavía para ESTE usuario en este
      // dispositivo (o el caché era de otro usuario): usa bootAfterLogin,
      // que a su vez revisa si ya existe caché propio antes de ir a la red.
  ...
  // Si no hay sesión pero sí había caché local, se deja ver la app con los
  // datos ya guardados en el dispositivo (nunca se resetea a una pantalla
  // en blanco); el usuario puede cerrar sesión manualmente desde Ajustes.
}
