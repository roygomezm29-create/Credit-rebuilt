...
/* ---------------------------- PERSISTENCIA LOCAL (localStorage-first) ----------------------------
   Arquitectura anti-pérdida-de-datos para iOS: cada cambio se escribe en
   localStorage de forma SÍNCRONA e INMEDIATA (writeLocalState), antes de
   siquiera intentar tocar la red. Supabase es un espejo/backup que se
   sincroniza justo después, y también al recuperar el foco (visibilitychange),
   al recuperar conexión (online) y al abrir la app — nunca dependiendo de
   beforeunload/cierre, que iOS no garantiza ejecutar.
   ...
   nada mientras tanto porque la fuente de verdad inmediata ya es localStorage. */
const LOCAL_KEY_PREFIX = "credit_v1_state_";
const localKeyFor = (userId) => `${LOCAL_KEY_PREFIX}${userId}`;

function readLocalState(userId) {
  try {
    const raw = localStorage.getItem(localKeyFor(userId));
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    return parsed && parsed.state ? parsed : null;
  } catch (e) {
    console.error("Error leyendo respaldo local", e);
    return null;
  }
}

function writeLocalState(userId, next, dirty) {
  try {
    localStorage.setItem(localKeyFor(userId), JSON.stringify({ state: next, dirty: !!dirty, updatedAt: Date.now() }));
  } catch (e) {
    console.error("Error guardando respaldo local (localStorage)", e);
  }
}
...
async function trySync() {
  ...
  const local = readLocalState(currentUser.id);
  ...
  if (ok) {
    ...
    const stillCurrent = readLocalState(currentUser.id);
    if (stillCurrent && stillCurrent.updatedAt === local.updatedAt) {
      writeLocalState(currentUser.id, local.state, false);
    }
  }
  syncInFlight = false;
}
...
/* Agrupa sincronizaciones muy seguidas (varios taps rápidos) en una sola
   llamada a Supabase, sin retrasar nunca el guardado local síncrono. */
...
function saveState(next) {
  state = next;
  render(); // UI optimista: se refleja de inmediato
  if (currentUser) writeLocalState(currentUser.id, next, true); // 1) guardado SÍNCRONO e inmediato en localStorage (fuente de verdad al instante)
...
async function bootAfterLogin(user) {
  ...
  if (local) {
    // Arranque instantáneo desde localStorage: es la fuente de verdad inmediata,
    // así que la app funciona aunque no haya red o Supabase tarde en responder.
    state = local.state;
    dataLoading = false;
    render();
    if (local.dirty) {
  ...
  writeLocalState(user.id, state, false);
  dataLoading = false;
  render();
}
...
async function refreshFromSupabaseInBackground(userId) {
  try {
    ...
    if (currentUser && currentUser.id === userId && (!local || !local.dirty)) {
      state = fresh;
      writeLocalState(userId, fresh, false);
      render();
    }
  } catch (e) {
